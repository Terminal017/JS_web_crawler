...
export class Crawler {
   ...
   * @param url 索引页URL
   */
  private async crawlIndexPage(url: string, currentPage = 1): Promise<void> {
    if (!this.context) throw new Error('浏览器上下文未初始化')
    if (!this.config.selectors.indexPage) throw new Error('未配置索引页选择器')
    ...
    try {
      ...
      // 获取所有索引页链接
      const indexUrls = await page.$$eval(
        items,
        (elements, baseUrl) => {
          return elements
            .map((el) => {
